...
class PlayList {
    ...
    public void remove(String title) {
        if (size > 0) {
            for (int i = 0; i < size; i++) {
                if (title == this.tracks[i].getTitle()) {
                    remove(i);
                }
            }
        }
    ...
    //// An elegant and terribly inefficient implementation.
     public void add(PlayList other) {
        int mutualSize = this.size + other.size;
        if (mutualSize <= this.maxSize) {
    ...
    public void sortedInPlace() {
        ...
        int indexToSwap = 0;
        for (int i = 0; i < size; i++) {
            indexToSwap = minIndex(i);
            tempTrack = tracks[i];
            tracks[i] = tracks[indexToSwap];
            tracks[indexToSwap] = tempTrack;
        }
    }
}
